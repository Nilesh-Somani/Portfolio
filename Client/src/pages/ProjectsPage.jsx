...
// Helper to parse start date for sorting
...
export default function ProjectsPage() {
  ...
  // Collect tech stack options
  const techOptions = useMemo(() => {
    const set = new Set();
    projects.forEach((p) => {
      if (Array.isArray(p.tech)) p.tech.forEach((t) => set.add(t));
    });
  ...
  const filtered = useMemo(() => {
    ...
    let list = projects.filter((p) => {
      ...
      const matchesTech = !filterTech
        ? true
        : p.tech &&
        p.tech.map((t) => t.toLowerCase()).includes(filterTech.toLowerCase());
    ...
    switch (sortBy) {
      ...
      case "oldest":
        list.sort(
          (a, b) => parseStartDate(a.duration) - parseStartDate(b.duration)
        );
        break;
      default:
        break;
      ...
      <div className="max-w-screen-xl mx-auto px-4">
        <div className="mb-8">
          ...
          <SearchBar
            ...
            placeholder="Search, filter, or sort projects..."
          />
        </div>
                ...
                <div className="p-6 flex flex-col flex-1">
                  {p.icon && (
                    <img
                      src={p.icon}
                      alt={p.title}
                      className="w-20 h-20 object-contain mb-4 self-center rounded-lg bg-gray-900 border border-gray-800"
                    />
                  )}
                  ...
                  <p className="text-gray-300 mt-3 flex-1">{p.brief}</p>
